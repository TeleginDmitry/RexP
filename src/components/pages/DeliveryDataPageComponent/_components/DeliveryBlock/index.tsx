...
const DeliveryBlock = () => {
  ...
  const onHandleClick = (id: number, typeId: number) => {
    router.push({ pathname: "/profile/deliveryDetails", query: { id, typeId } }, undefined, { shallow: true });
  };
  ...
  return (
    <div className={s.wrapper}>
      {deliveryCarts.length &&
        deliveryCarts.map(
          ({ id, firstName, lastName, patronymic, number, deliveryType, isMain, deliveryPointAddress }, index) => (
            <div className={clsx(s.address, isMain && s.active)} key={index}>
              <div className={s.header}>
                <div className={s.title}>{isMain ? "Основной адрес" : "Дополнительный адрес"}</div>
                <RootButton onClick={() => onHandleClick(id!, deliveryType!.id)}>
              ...
              <div className={s.content}>
                ...
                <div className={s.phone}>{number?.replace(/(\d)(\d{3})(\d{3})(\d{2})(\d{2})/, "+$1 $2 $3 $4 $5")}</div>
                <div className={s.title}>Адрес доставки</div>
                <div className={s.addressValue}>{deliveryPointAddress}</div>
              </div>
              {!isMain && (
                <Button className={clsx(s.button)} onClick={() => onHandleActiveClick(id!)}>
                  Сделать основным
                </Button>
              )}
            </div>
