...
const DeliveryBlock = () => {
  ...
  const onHandleClick = (deliveryType: string) => {
    router.push(
      { pathname: "/profile/deliveryDetails", query: { id: addressActiveValue, type: deliveryType } },
      undefined,
      { shallow: true }
    );
  };
  ...
  return (
    <div className={s.wrapper}>
      {addressesValue &&
        JSON.parse(addressesValue).map(
          ({ id, name, surname, patronymic, phone, deliveryType, city, street, house, flat, pvzAddress }) => (
            <div className={clsx(s.address, id === addressActiveValue && s.active)} key={id}>
              <div className={s.header}>
                <div className={s.title}>Основной адрес</div>
                <RootButton onClick={() => onHandleClick(deliveryType)}>
              ...
              <div className={s.content}>
                ...
                <div className={s.addressValue}>
                  {deliveryType === "Курьером" ? (
                    <>
                      {deliveryType}, {city}, {street}, {`Д. ${house}`}, {`Кв. ${flat}`}
                    </>
                  ) : (
                    <>
                      {deliveryType}, {city}, {pvzAddress}
                    </>
                  )}
                </div>
              </div>
              {addressActiveValue !== id && (
                <Button className={clsx(s.button)} onClick={() => setAddressesActive(id)}>
                  Сделать основным
                </Button>
              )}
            </div>
