...
export const Selector = ({ values, defaultValue, onChange }: Props) => {
  ...
  return (
    ...
    >
      <span className="flex items-center gap-2" onClick={() => setIsVisibleSelect((state) => !state)}>
        {defaultValue ?? "Выбрать"}{" "}
        {isVisibleSelect ? (
          <svg xmlns="http://www.w3.org/2000/svg" width="12" height="8" viewBox="0 0 10 6" fill="none">
            <path d="M1 5L5 0.999969L9 5" stroke="white" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
        ) : (
          <svg xmlns="http://www.w3.org/2000/svg" width="12" height="8" viewBox="0 0 10 6" fill="none">
            <path d="M9 1L5 5.00003L1 1" stroke="white" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
        )}
      </span>
