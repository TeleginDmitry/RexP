  ...
  return (
    <>
      ...
      <MainContainer>
        ...
        <div className="mt-10">
          {orders.map(({ delivery, totalPrice, id, createdAt, orderContents }) => {
            const createdAtDate = new Date(createdAt).toLocaleDateString("ru-RU", { day: "numeric", month: "long" });

            const imagesFull = orderContents?.map(({ product }) => product.images).flat();

            return (
              <div key={id} className="w-full p-6 rounded-3xl bg-[#EEE] flex flex-col gap-5 mt-4">
                <div className="flex items-center justify-between pb-3 border-b border-[rgba(142,142,142,0.40)] border-solid w-full">
                  <p className="text-base">
                    Заказ от <span className="font-semibold">{createdAtDate}</span>
                  </p>
                  <span className="text-base text-[#8E8E8E]">{id}</span>
                </div>
                ...
                <button onClick={handleVisibleModal} className="p-4 bg-black rounded-xl text-white font-bold text-base">
                  Оставить отзыв
                </button>
              </div>
            );
      ...
      <Modal isOpen={isVisibleModal} onClose={handleVisibleModal}>
        <div className="fixed top-0 left-0 z-50 p-4 flex justify-center items-center w-full h-full bg-black bg-opacity-40">
          <div className="rounded-xl max-w-72 bg-[rgba(179,179,179)] flex flex-col gap-2">
            <div className="px-4 pt-4 flex flex-col gap-1 items-center">
              <h2 className="text-base font-semibold text-center">Хотите оставить отзыв на заказ?</h2>
              <p className="text-center text-sm">
                Для того, чтобы оставить отзыв на полученный заказ, необходимо перейти в бот для отзывов.
              </p>
            </div>
            <div>
